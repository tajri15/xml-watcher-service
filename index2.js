...
const fixImageUrlsForSplitContainer = (imgtypeData, splitFolder, basePath) => {
  ...
  // Pattern 1: Deteksi URL dengan format lengkap untuk FS02
  // http://192.111.111.80:6688/62001FS02/2025/1118/0508/62001FS03202511180508.jpg
  const fullUrlPattern = /(http:\/\/192\.111\.111\.80:6688)(\/62001FS02\/\d{4}\/\d{4}\/\d{4}\/)([^\/<>"\s]+\.(?:jpg|img|_icon\.jpg))/gi;
  
  // Pattern 2: Deteksi URL relatif dalam CDATA untuk FS02
  const relativeUrlPattern = /(&lt;img&gt;)(http:\/\/192\.111\.111\.80:6688)(\/62001FS02\/\d{4}\/\d{4}\/\d{4}\/)([^\/<>"\s]+\.(?:jpg|img|_icon\.jpg))(&lt;\/img&gt;)/gi;
...
// ============================================================
// FUNGSI GENERATE FTP PATH - DIUBAH UNTUK EXPORT
// ============================================================
const getFtpPath = (filePath, splitFolder) => {
  try {
    const relativePath = path.dirname(path.relative(WATCH_PATH, filePath));
    const basePath = '/export/62001FS02';  // DIUBAH DARI import KE export
    ...
    return '/export/62001FS02/';  // DIUBAH DARI import KE export
  }
};
...
const processAndSendXml = async (filePath) => {
  try {
    ...
    console.log(`[SEND] URL: ${POST_URL}`);  // SUDAH BENAR UNTUK EXPORT
